...
<body>
	...
	<form name="AnyadirForm" action="shopping" method="POST">
		<input type="hidden" name="todo" value="add"> Titulo: <select
			name=idLibro>
			<%LibrosBD.cargarDatos();
				// Scriptlet 1: Carga los libros en el select.
				for (int i = 0; i < LibrosBD.tamanyo(); i++) {
					out.println("<option value='" + i + "'>");
					out.println(LibrosBD.getTitulo(i) + " | " + LibrosBD.getAutor(i) + " | " + LibrosBD.getPrecio(i));
					out.println("</option>");
				}
			%>
		</select> Cantidad <input type="text" name="cantidad" size="10" value="1"><input type="submit"
			value="Anyadir a la cesta">
	</form>
	<br />
	<hr />
	<br />
