...
// crea un elementoPedido y lo a~nade al carrito
			...
			cantidadActualEnStock = LibrosBD.getStock(nuevoElementoPedido.getIdLibro());
			if (nuevoElementoPedido.getCantidad() > cantidadActualEnStock) {
				nextPage = "/error.jsp";
			} else {
				int actualizarCantidad = cantidadActualEnStock - nuevoElementoPedido.getCantidad();
				LibrosBD.actualizarStock(actualizarCantidad, nuevoElementoPedido.getAutor());
...
// Borra el elemento indiceElemento del carrito
			...
			cantidadActualEnStock=LibrosBD.getStock(indiceCarrito)+elCarrito.get(indiceCarrito).getCantidad();
			LibrosBD.actualizarStock(cantidadActualEnStock, LibrosBD.getAutor(indiceCarrito));
